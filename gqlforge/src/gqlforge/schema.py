...
def discover_sources(config: Config) -> list[str]:
    """Return committed schema sources ordered newest-first."""
    sources = [
        s
        for s in config.source_order
        if (config.schemas_dir / f"{s}.graphql").is_file()
    ]
    if not sources:
        raise GqlforgeError(f"No schema files found under {config.schemas_dir}")
    return sources


def load_schema_document(config: Config, source: str) -> DocumentNode:
    """Parse a committed schema SDL file into an AST document."""
    path = config.schemas_dir / f"{source}.graphql"
...
def download_schema_sdl(base_url: str, token: str, timeout: float = 60.0) -> str:
    ...
    if payload.get("errors"):
        raise GqlforgeError(f"Introspection errors: {payload['errors']}")
