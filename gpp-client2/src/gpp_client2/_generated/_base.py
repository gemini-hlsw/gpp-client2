...
__all__ = ["UNSET", "Input", "Model", "UnsetType", "is_set"]
...
class UnsetType:
    ...
    Distinct from ``None``, which means the server returned null. Falsy,
    so truthiness checks behave sensibly either way.
    """
...
class Model(BaseModel):
    ...
    Every field defaults to ``UNSET``; a parsed response sets only the
    fields the operation selected. ``repr`` shows set fields only.
    """
...
class Input(BaseModel):
    ...
    Serialization sends only fields that were explicitly set, so
    "omitted" and "explicitly null" stay distinct - exactly GraphQL's
    semantics.
    """
