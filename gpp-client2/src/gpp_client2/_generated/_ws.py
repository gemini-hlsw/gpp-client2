...
transports add only the actual WebSocket I/O - ``websockets`` ships both
a sync and an asyncio client - so sync and async subscription behavior
cannot drift. Each subscription call opens its own connection: one
socket, one ``subscribe``, closed when iteration ends.

Event payloads flow through the same root-null partial-response
semantics as HTTP responses.
"""
...
def get_ws_url(base_url: str) -> str:
    """WebSocket endpoint for an HTTP endpoint: scheme swap, same path."""
...
class WsConfig:
    ...
    url : str
        The ``ws(s)://`` endpoint.
    token : str
        Bearer token, sent in the upgrade request and the
        ``connection_init`` payload; empty for anonymous servers.
...
class _Protocol:
    ...
    def init_message(self) -> str:
        payload = {"Authorization": f"Bearer {self._token}"} if self._token else {}
    ...
    def handle(self, raw: str | bytes) -> tuple[str, Any]:
        ...
        Returns an ``(action, value)`` pair: ``("ack", None)``,
...
def _map_ws_error(exc: Exception, url: str) -> Exception:
    ...
    if isinstance(exc, InvalidStatus):
        ...
        return TransportError(f"WebSocket handshake with {url} failed: HTTP {code}.")
    ...
    if isinstance(exc, TimeoutError):
        return RequestTimeoutError(f"Connecting to {url} timed out: {exc}")
